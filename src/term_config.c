...
cfgStruct cfg[] =
{
	...
	{"newline", CFG_BOOL, &newline},
	{"creturn", CFG_BOOL, &creturn},
...
void ConfigFlags(void)
{
	...
	Set_newline(config.newline);
	Set_creturn(config.creturn);
...
{
	GtkWidget *Table, *Label, *Bouton_APPLY, *Bouton_OK, *Bouton_annule,
	...
	gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(Combo), "74880");
	...
	case 1000000:
		...
		break;
	case 2000000:
		gtk_combo_box_set_active(GTK_COMBO_BOX(Combo), 16);
		break;
	case 0:
	...
	Bouton_APPLY = gtk_button_new_from_stock(GTK_STOCK_APPLY);
	gtk_box_pack_start(GTK_BOX(action_area), Bouton_APPLY, FALSE, TRUE, 0);
	g_signal_connect(GTK_WIDGET(Bouton_APPLY), "clicked", G_CALLBACK(Lis_Config), (gpointer)Combos);
		...
		for(i = 0; i < max; i++)
		{
			if(!strcmp(config_name, cfgSectionNumberToName(i)))
			{
				...
				if(crlfauto[i] != -1)
				 ...
				 if(newline[i] != -1)
					config.newline = (gboolean)newline[i];
				else
					config.newline = FALSE;

				 if(creturn[i] != -1)
					config.creturn = (gboolean)creturn[i];
				else
...
void Copy_configuration(int pos)
{
	...
	cfgStoreValue(cfg, "newline", string, CFG_INI, pos);
	g_free(string);
	
	if(config.creturn == FALSE)
		string = g_strdup_printf("False");
	else
		string = g_strdup_printf("True");

	cfgStoreValue(cfg, "creturn", string, CFG_INI, pos);
	g_free(string);
